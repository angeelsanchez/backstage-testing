...
export const BulletinBoardPage = () => {
  ...
  const findCurrentCard = () =>
    allCards.find((item: Bulletin) => item.bulletin_id === currentCardId);
  ...
  const getAvailableTags = (): Array<string> => {
    return allCards
      .reduce((allTags: Array<string>, itemArr: Bulletin) => {
  ...
  const deleteCard = async (id: string) => {
    const res: Awaited<Promise<any>> = await bulletinBoardApi.deleteBulletin(
      id,
    );
    if (res.status === 'ok') {
      fetchCards();
  ...
  return (
    <Page themeId="home">
      <Header
        ...
        subtitle="Un lugar básico para compartir ideas y enlaces con su equipo."
      />
      <Content>
        <FormDialog
          status={open}
        ...
        <Box className={classes.contentBox}>
          <BulletinCard
            ...
            deleteCard={deleteCard} // Agregar esta línea
          />
          <Pagination
